...
app = FastAPI(title="JWT Refresh + SQLite", lifespan=lifespan)
...
def create_refresh_token(db: Session, user: CurrentUser) -> str:
...
def create_token_pair(db: Session, user: CurrentUser) -> TokenPair:
    return TokenPair(
...
def login(
    ...
    return create_token_pair(db, current_user)
...
def refresh_token(payload: RefreshRequest, db: Annotated[Session, Depends(get_db)]):
    ...
    if saved_token is None or saved_token.expires_at < int(datetime.now(UTC).timestamp()):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    current_user = CurrentUser(id=user.id, username=user.username)
    return AccessToken(access_token=create_access_token(current_user))
