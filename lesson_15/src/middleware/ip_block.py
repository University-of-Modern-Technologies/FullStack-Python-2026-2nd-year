"""Middleware для блокування HTTP-запитів за IP-адресою."""
import json
import logging
...
class BlockedIpEntry(BaseModel):
    """Pydantic-модель одного запису у JSON-списку заблокованих IP."""
    ip: str
    reason: str


def load_blocked_ips(path: Path) -> dict[str, str]:
    """Завантажує JSON-список заблокованих IP у словник причин."""
    if not path.is_file():
...
class IpBlockMiddleware(BaseHTTPMiddleware):
    """Starlette middleware, що відхиляє запити з заблокованих IP."""
    ...
    async def dispatch(self, request: Request, call_next):
        """Перевіряє IP клієнта перед передачею запиту наступному handler-у."""
